...
homePath = pathtofile(1:(regexp(pathtofile,'DDMAnalysis_BaseModel') - 1));
if isempty(homePath) % catch bug on Dropbox where folders are lowercase
    homePath = pathToFile(1:(regexp(pathToFile,lower('DDMAnalyis_BaseModel')) - 1));
end
%
...
Model_Details.block_lower{1}=[1 2 3 4 5];   % These are for individual level parameters (C.H.: 5 in total for this version)
...
Nchain=15;                     % Number of MCMC chains. Use 3 x largest block size
...
DIR = fullfile(homePath,'DDMAnalysis_Bayes_BaseModel','fittedData'); % C.H.
if ~exist(DIR, 'dir')
    mkdir(DIR)
end
