...
initWeights = [params.initTaste params.initHealth params.initConstant];
finalWeights = [params.finalTaste params.finalHealth params.finalConstant];

if params.timeShift == 0
    finalDrifts = [Taste Health ones(length(Taste),1)]*finalWeights';
    initDrifts = finalDrifts; % initial and final are same thing with no shift       
else
    initDrifts = [Taste Health ones(length(Taste),1)]*initWeights';
    finalDrifts = [Taste Health ones(length(Taste),1)]*finalWeights';
end
