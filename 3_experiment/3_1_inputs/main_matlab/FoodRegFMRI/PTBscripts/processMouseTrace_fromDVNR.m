...
%
% normedTrajectory2: like normed trajectory, but replaces points of 0
% movement with previous angle
%
...
currentAnglev2 = currentAngle; % calculate second version where moments of pause are replaced with previous angle
currentAngle(isnan(currentAngle)) = 0;

mousePaused = find(isnan(currentAnglev2));
mousePaused(mousePaused == 1) = [];
for tPt = mousePaused
    currentAnglev2(tPt) = currentAnglev2(tPt - 1);
end
...
if ~exist('EndPtCtrs','var') % use normalized trajectory
    ...
    yDiff = diff(normY);
    currentAngle = asind(xDiff./sqrt(xDiff.^2 + yDiff.^2));
...
%     currentAngle(yDiff < 0 & xDiff < 0) = 360 - currentAngle(yDiff < 0 & xDiff < 0);
    currentAngle(yDiff < 0 & normY(2:end) < 1) = NaN;
    currentAngle(xDiff < .01 & yDiff == 0) = NaN;
    
    % clean singleton spikes
    aDiff = diff(currentAngle);
    spikes = find(abs(aDiff) > 75) + 1;
    for tPt = spikes'
        currentAngle(tPt) = currentAngle(tPt -1);
    end
    
    AngleToRight = asind((1 - normXSigned)./sqrt((1 - normXSigned).^2 + (1 - normY).^2));
    AngleToLeft = asind((-1 - normXSigned)./sqrt((-1 - normXSigned).^2 + (1 - normY).^2));
    ...
    currentAnglev2 = currentAngle;
    currentAngle(isnan(currentAngle)) = AngleBisect(isnan(currentAngle));
    
    % replace points of no movement with previous angle
    mousePaused = find(isnan(currentAnglev2));
    if any(mousePaused == 1)
        currentAnglev2(1) = 0;
        mousePaused(mousePaused == 1) = [];
    end
    for tPt = mousePaused'
    ...
    normedTrajectory2 = (currentAnglev2 - AngleBisect)./AngleDiff;
    normedTrajectory(end + 1) = normedTrajectory(end); % add in 1 extra pt at end
    normedTrajectory2(end + 1) = normedTrajectory2(end); % add in 1 extra pt at end
    ...
    normedTrajectory2(lastBeforeFinalMove:end) = sign(normXSigned(lastBeforeFinalMove));
    % replace moves outside of -1 to 1 with -1 or 1
    normedTrajectory3 = normedTrajectory2;
    ...
    for i = find(abs(normedTrajectory2) > 1)'
        if i > 1
            normedTrajectory2(i) = sign(normedTrajectory2(i)); % -1 or 1
        else
            normedTrajectory2(i) = AngleBisect(i);
        end
    end
    
    for i = find(abs(normedTrajectory3) > 1)'
        if i == 1
            normedTrajectory3(i) = AngleBisect(i);
        end
    end
    
    % replace singleton spikes in normed trajectory
    aDiff = diff(normedTrajectory);
    spikes = find(abs(aDiff) > .75) + 1;
    for tPt = spikes'
        normedTrajectory(tPt) = normedTrajectory(tPt -1);
    end
    
    normedTrajectory100 = interp1(1:length(normedTrajectory),normedTrajectory,linspace(1,length(normedTrajectory),100))';
    normedTrajectory2100 = interp1(1:length(normedTrajectory2),normedTrajectory2,linspace(1,length(normedTrajectory2),100))';
    normedTrajectory3100 = interp1(1:length(normedTrajectory3),normedTrajectory3,linspace(1,length(normedTrajectory3),100))';
end
...
varnames = {'RT','normX','normY','normXSigned','finalChoice','tracetime'...
            ...
            'AUCTotal','AUCToOther','AUCToSelf','maxDev','normedTrajectory2',...
            'normedTrajectory2100','normedTrajectory3','normedTrajectory3100'...
           };
