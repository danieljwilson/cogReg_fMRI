...
if str2num(PTBParams.ssnid) == 1
    blockorder = [1 randperm(2) + 1]; % always start with nat response on first session
else
...
for block = blockorder % for each session, run one block of each condition
    switch block
        case 1
            ...
            Food = FoodOrderNat(10*(str2num(PTBParams.ssnid) - 1) + (1:10)); % select the appropriate trials
            Insrx = 'Respond Naturally';
        case 2
            InsrxPic = PTBParams.RegInsrx1;
            InsrxSize = PTBParams.RegPicSize1;
            Food = FoodOrderReg1(10*(str2num(PTBParams.ssnid) - 1) + (1:10));
            Insrx = 'Focus on Healthiness';
        otherwise
            InsrxPic = PTBParams.RegInsrx2;
            InsrxSize = PTBParams.RegPicSize2;
            Food = FoodOrderReg2(10*(str2num(PTBParams.ssnid) -1) + (1:10));
...
function determineFoodOrder(PTBParams)
    ...
    FoodOrderNat = FoodOrderNat(1:90);
    FoodOrderReg1 = FoodOrderReg1(1:90);
    ...
    datafile = PTBParams.datafile;
    logData(datafile,1,FoodOrderNat,FoodOrderReg1, FoodOrderReg2);
    save(fullfile(PTBParams.homepath, 'SubjectData', num2str(PTBParams.subjid), 'TaskOrder.mat'), 'FoodOrderNat','FoodOrderReg1','FoodOrderReg2')
