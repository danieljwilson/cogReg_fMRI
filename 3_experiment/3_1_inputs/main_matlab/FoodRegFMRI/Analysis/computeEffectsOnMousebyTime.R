computeEffectsOnMouseByTime = function(Data, TrialData, mouseVarName = "MouseX", adjustToFirstDevVar = 'none', 
  ...
  if(all(trialIDsMouse %in% trialIDsData) & all(trialIDsData %in% trialIDsMouse)){# run analyses if trial identifiers match up
        ...
        Betas = Betas_SE= Pvals = matrix(data = NA, nrow = maxTime, ncol = length(PredictorNames) + 1)
        colnames(Betas) = c(PredictorNames,'Intercept')
        colnames(Betas_SE) = c(PredictorNames,'Intercept')
        colnames(Pvals) = c(PredictorNames,'Intercept')
...
#           print(time)
          ...
          if(!any(is.na(m$coefficients)) ){ # is.na(m$coefficients['(Intercept)', 't value'])
            if(lmType == 'binomial' && logF){ # if penalized logistic was used
              ...
              Betas[time,length(colnames(Betas))] = m$coefficients[1]
              Betas_SE[time,length(colnames(Betas))] = m$var[1]^.5 # check
              Pvals[time,length(colnames(Betas))] = m$prob[1]
...
#                 }
              ...
              # Intercept term
              Betas[time,length(colnames(Betas))] = m$coefficients['(Intercept)','Estimate']
              Betas_SE[time,length(colnames(Betas))] = m$coefficients['(Intercept)','Std. Error']

              if(lmType == 'binomial'){
                Pvals[time,length(colnames(Betas))] = tryCatch(m$coefficients['(Intercept)','Pr(>|z|)']
                           ,error = function(e){NA})
              }else{
                Pvals[time,length(colnames(Betas))] = tryCatch(m$coefficients['(Intercept)','Pr(>|t|)']
                           ,error = function(e){NA})
              }
            }
          }else{
            ...
            Betas[time,length(colnames(Betas))] = NA
            Betas_SE[time,length(colnames(Betas))] = NA
            Pvals[time,length(colnames(Betas))] = 1
          }
        }
        ...
        for(p in 1:(length(PredictorNames) + 1)){
          if(!returnP){
            data = matrix(0,3,maxTime)
          }else{
            data = matrix(0,4,maxTime)
          }
          if(p <= length(PredictorNames)){
            ...
            data[1,] = Betas[,'Intercept']
            data[2,] = Betas[,'Intercept'] - Betas_SE[,'Intercept']
            data[3,] = Betas[,'Intercept'] + Betas_SE[,'Intercept']
            if(returnP){
              data[4,] = Pvals[,'Intercept']
            }
            PredData[['Intercept']] = data
          }
        }
