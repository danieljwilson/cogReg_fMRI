...
def runShiftingLetterNumberBlock(taskName='shiftingLetterNumber', blockType='', trials=10, feedback=False, saveData=True, practiceTrials=10, switchProportion=0.3, titrate=False, rtMaxFrames=None, blockMaxTimeSeconds=None, experimentMaxTimeSeconds=None, rewardSchedule=3, feedbackSound=False, pauseAfterMissingNTrials=None):
    ...
    for i, thisTrial in trialsDf.iterrows(): #for each trial...
        ...
        if thisTrial.trialNo == 1:
            ...
            while(not event.getKeys(keyList=['5'])):
                time.sleep(0.001)
            while(not event.getKeys(keyList=['5'])): # the 4th TR launches the first trial
                time.sleep(0.001)
...
"The background color (blue or green) of the pairing is an additional cue you can rely on to know whether to focus on the letter or number. So try to rely on the color cues to get better at the task.",
...
# actual
runShiftingLetterNumberBlock(taskName='shiftingLetterNumber', blockType='actual', trials=80, feedback=False, saveData=True, practiceTrials=10, switchProportion=0.5, titrate=False, rtMaxFrames=60 * Hz, blockMaxTimeSeconds=None, experimentMaxTimeSeconds=None, rewardSchedule=1, feedbackSound=False, pauseAfterMissingNTrials=5)

''' experiment end '''
showInstructions(text=["That's the end of the experiment. Thanks for the good work!"])
