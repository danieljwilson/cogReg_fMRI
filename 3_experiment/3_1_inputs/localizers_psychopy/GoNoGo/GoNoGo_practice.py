...
def showInstructions(text, timeBeforeAutomaticProceed = 0, timeBeforeShowingSpace = 0):
    ...
    continueText = visual.TextStim(win = win, units = 'norm', colorSpace = 'rgb', color = [1, 1, 1], font = 'Verdana', text = "Press button 1 to continue", height = 0.06, wrapWidth = 1.4, pos = [0.0, 0.0])
...
def runBlockLow(blockType = '', reps = 1, feedback = False, saveData = True, practiceTrials = 10):
    ...
    try:
        ...
        blockNumber = 1
        trialsDf['blockNumber'] = blockNumber
        # IF first trial, then wait for MRI signal '5's
...
def showTaskInstructions():
    ...
    "The letter will disappear after {:.2f} seconds, so you'll have to respond quickly.".format(float(info['targetRemoveFrames']) / 60),
    ...
    # You'll have opportunities to take short breaks during study.

def showBlockBeginInstructions():
    showInstructions(text =
    ["Get ready and place your finger on the button.",
    "{:.2f} seconds to respond.".format(float(info['targetFrames']) / 60),
...
#end task
showInstructions(text = ["You've completed your practice. Well done."])
