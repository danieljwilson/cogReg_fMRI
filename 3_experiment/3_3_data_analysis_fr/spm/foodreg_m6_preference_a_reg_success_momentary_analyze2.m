function foodreg_m6_preference_a_reg_success_momentary_analyze2(subjNameList, preproc_version)
...
%  post_food = post_ratings.Food(post_liking_trials);
    ...
    % calculated expected rating based on Natural trials lm
    load(fullfile(homepath, studyid, 'trial_data', 'sub_lms.mat'));
    intercept = sub_lms(sub_lms(:,1)==str2num(subjID), 2);
    pre_liking_beta = sub_lms(sub_lms(:,1)==str2num(subjID), 3);
    ...
    for session=1:length(files) % this loops over sessions
        ...
        % Second modulator
        ons_modulate{1,session}{2}.name='reg_success_momentary';   
        ons_modulate{2,session}{2}.name='reg_success_momentary';
        ons_modulate{3,session}{2}.name='reg_success_momentary';
        ...
        ons_modulate{4,session}{1}.name='none'; % DO I NEED TO ADD A SECOND
        ...
        % momentary regulatory success modulator
        reg_success_momentary_Natural = zeros(sum(Natural),1);
        ...
        for i = 1:sum(Natural)
            ...
            expected_rating = intercept + pre_liking_beta * pre_rating(trial_food_pre);
            rating_change = expected_rating - pref_Natural(i);
            if health_rating_trial > 3
                reg_success_momentary_Natural(i) = -rating_change;
            else
                reg_success_momentary_Natural(i) = rating_change;
            end
        end
        
        reg_success_momentary_DecreaseDesire = zeros(sum(DecreaseDesire),1);
        ...
        for i = 1:sum(DecreaseDesire)
            ...
            rating_change = expected_rating - pref_Natural(i);
            if health_rating_trial > 3
                reg_success_momentary_DecreaseDesire(i) = -rating_change;
            else
                reg_success_momentary_DecreaseDesire(i) = rating_change;
            end
        end
        
        reg_success_momentary_HealthFocus = zeros(sum(HealthFocus),1);
        ...
        for i = 1:sum(HealthFocus)
            ...
            rating_change = expected_rating - pref_Natural(i);
            if health_rating_trial > 3
                reg_success_momentary_HealthFocus(i) = -rating_change;
            else
                reg_success_momentary_HealthFocus(i) = rating_change;
            end
        end
            
        ons_modulate{1,session}{2}.vec = reg_success_momentary_Natural;
        ons_modulate{2,session}{2}.vec = reg_success_momentary_DecreaseDesire;
        ons_modulate{3,session}{2}.vec = reg_success_momentary_HealthFocus;
