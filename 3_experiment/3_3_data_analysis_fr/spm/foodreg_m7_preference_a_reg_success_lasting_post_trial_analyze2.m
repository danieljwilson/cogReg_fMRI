function foodreg_m7_preference_a_reg_success_lasting_post_trial_analyze2(subjNameList, preproc_version)
...
ons_name{4}='Natural_post';
ons_name{5}='DecreaseDesire_post';
ons_name{6}='HealthFocus_post';
ons_name{7}='Missed Trials';
...
% convert to TSV from Matlab
    ...
    health_trials = strcmp(post_ratings.Attribute, 'Health');
    
    % get rating and food item for pre, post and health
    pre_rating = cell2mat(pre_ratings.Resp);
    post_rating = cell2mat(post_ratings.Resp(post_liking_trials));
    health_rating = cell2mat(post_ratings.Resp(health_trials));
    pre_food = pre_ratings.Food;
    post_food = post_ratings.Food(post_liking_trials);
    health_food = post_ratings.Food(health_trials);
    ...
    for session=1:length(files) % this loops over sessions
        ...
        ons{4,session}= ons_Natural' + RT_Natural';
        ons{5,session}= ons_DecreaseDesire' + RT_DecreaseDesire';
        ons{6,session}= ons_HealthFocus' + RT_HealthFocus';
        ons{7,session}= ons_Missed;
        ...
        ons_duration{4,session}= 2; %in secs
        ons_duration{5,session}= 2; %in secs
        ons_duration{6,session}= 2; %in secs
        ons_duration{7,session}= 4;  % this is for missed trials (get 4 s)
        ...
        ons_modulate{4,session}{1}.name='Preference';
        ons_modulate{5,session}{1}.name='Preference';
        ons_modulate{6,session}{1}.name='Preference';
        
        % Second modulator
        ons_modulate{1,session}{2}.name='reg_success_lasting';   
        ons_modulate{2,session}{2}.name='reg_success_lasting';
        ons_modulate{3,session}{2}.name='reg_success_lasting';
        ons_modulate{4,session}{2}.name='reg_success_lasting';   
        ons_modulate{5,session}{2}.name='reg_success_lasting';
        ons_modulate{6,session}{2}.name='reg_success_lasting';
        ...
        ons_modulate{7,session}{1}.name='none'; % DO I NEED TO ADD A SECOND
        ...
        ons_modulate{1,session}{1}.vec = pref_Natural';
        ons_modulate{2,session}{1}.vec = pref_DecreaseDesire';
        ons_modulate{3,session}{1}.vec = pref_HealthFocus';
        ons_modulate{4,session}{1}.vec = pref_Natural';
        ons_modulate{5,session}{1}.vec = pref_DecreaseDesire';
        ons_modulate{6,session}{1}.vec = pref_HealthFocus';
        
        % lasting regulatory success modulator
        reg_success_lasting_Natural = zeros(sum(Natural),1);
        health_HealthFocus = zeros(sum(Natural),1);

        trial_foods = Data.Food(Natural);
        for i = 1:sum(Natural)
            trial_food_pre = strcmp(pre_food, trial_foods(i));
            trial_food_post = strcmp(post_food, trial_foods(i));
            trial_food = strcmp(health_food, trial_foods(i));
            health_rating_trial = health_rating(trial_food);
            pre_m_post_trial = pre_rating(trial_food_pre) - post_rating(trial_food_post);
            reg_success_lasting_Natural(i) = pre_m_post_trial;
        end
        
        reg_success_lasting_DecreaseDesire = zeros(sum(DecreaseDesire),1);
        health_HealthFocus = zeros(sum(DecreaseDesire),1);
        ...
        for i = 1:sum(DecreaseDesire)
            ...
            health_rating_trial = health_rating(trial_food);
            pre_m_post_trial = pre_rating(trial_food_pre) - post_rating(trial_food_post);
            reg_success_lasting_DecreaseDesire(i) = pre_m_post_trial;
        end
        
        reg_success_lasting_HealthFocus = zeros(sum(HealthFocus),1);
        health_HealthFocus = zeros(sum(HealthFocus),1);
        ...
        for i = 1:sum(HealthFocus)
            ...
            health_rating_trial = health_rating(trial_food);
            pre_m_post_trial = pre_rating(trial_food_pre) - post_rating(trial_food_post);
            if health_rating_trial > 3
                reg_success_lasting_HealthFocus(i) = -pre_m_post_trial;
            else
                reg_success_lasting_HealthFocus(i) = pre_m_post_trial;
            end
        end
            
        ons_modulate{1,session}{2}.vec = reg_success_lasting_Natural;
        ons_modulate{2,session}{2}.vec = reg_success_lasting_DecreaseDesire;
        ons_modulate{3,session}{2}.vec = reg_success_lasting_HealthFocus;
        ons_modulate{4,session}{2}.vec = reg_success_lasting_Natural;
        ons_modulate{5,session}{2}.vec = reg_success_lasting_DecreaseDesire;
        ons_modulate{6,session}{2}.vec = reg_success_lasting_HealthFocus;
