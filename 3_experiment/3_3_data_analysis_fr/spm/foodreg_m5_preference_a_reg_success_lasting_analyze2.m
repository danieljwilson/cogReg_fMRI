function foodreg_m5_preference_a_reg_success_lasting_analyze2(subjNameList, preproc_version)
...
% convert to TSV from Matlab
    ...
    for session=1:length(files) % this loops over sessions
        ...
        for i = 1:sum(Natural)
            ...
            post_m_pre_trial = pre_rating(trial_food_pre) - post_rating(trial_food_post);
            if health_rating_trial > 3
                reg_success_lasting_Natural(i) = -post_m_pre_trial;
            else
                reg_success_lasting_Natural(i) = post_m_pre_trial;
            end
        end
        ...
        for i = 1:sum(DecreaseDesire)
            ...
            post_m_pre_trial = pre_rating(trial_food_pre) - post_rating(trial_food_post);
            if health_rating_trial > 3
                reg_success_lasting_DecreaseDesire(i) = -post_m_pre_trial;
            else
                reg_success_lasting_DecreaseDesire(i) = post_m_pre_trial;
            end
        end
        ...
        for i = 1:sum(HealthFocus)
            ...
            post_m_pre_trial = pre_rating(trial_food_pre) - post_rating(trial_food_post);
            if health_rating_trial > 3
                reg_success_lasting_HealthFocus(i) = -post_m_pre_trial;
            else
                reg_success_lasting_HealthFocus(i) = post_m_pre_trial;
            end
        end
