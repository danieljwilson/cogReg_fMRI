function foodreg_m4_preference_a_pre_m_post_analyze2(subjNameList, preproc_version)
...
% convert to TSV from Matlab
    ...
    for session=1:length(files) % this loops over sessions
        ...
        % Second modulator
        ons_modulate{1,session}{2}.name='pre_m_post';   
        ons_modulate{2,session}{2}.name='pre_m_post';
        ons_modulate{3,session}{2}.name='pre_m_post';
        ...
        ons_modulate{1,session}{2}.vec = pre_m_post_Natural;
        ons_modulate{2,session}{2}.vec = pre_m_post_DecreaseDesire;
        ons_modulate{3,session}{2}.vec = pre_m_post_HealthFocus;
