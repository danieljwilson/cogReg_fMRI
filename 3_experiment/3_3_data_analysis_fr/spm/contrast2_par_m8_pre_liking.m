...
%                 };
    ...
    for subjno = 1:length(subj_ids)
        ...
        c{cnum}(searchcell(SPM.xX.name,'Naturalxpre_liking','contains')) = 1;
        c{cnum} = filterSessions(SPM, c{cnum}, subj_ids{subjno});
        cname{cnum}='Natural-pre_liking';
        ...
        cnum = length(c)+1;
        c{cnum}    = zeros(1,length(SPM.xX.name));
        c{cnum}(searchcell(SPM.xX.name,'DecreaseDesirexpre_liking','contains')) = 1;
        c{cnum} = filterSessions(SPM, c{cnum}, subj_ids{subjno});
        cname{cnum}='Decrease-pre_liking';
        ...
        cnum = length(c)+1;
        c{cnum}    = zeros(1,length(SPM.xX.name));
        c{cnum}(searchcell(SPM.xX.name,'HealthFocusxpre_liking','contains')) = 1;
        c{cnum} = filterSessions(SPM, c{cnum}, subj_ids{subjno});
        cname{cnum}='Health-pre_liking';
        ...
        cnum = length(c)+1;
        c{cnum}    = zeros(1,length(SPM.xX.name));
        c{cnum}    = c{searchcell(cname,'Health-pre_liking')} + c{searchcell(cname,'Natural-pre_liking')} + c{searchcell(cname,'Decrease-pre_liking')}; 
        cname{cnum}='All-pre_liking';
               
        cnum = length(c)+1;
        c{cnum}    = zeros(1,length(SPM.xX.name));
        c{cnum}    = c{searchcell(cname,'Decrease-pre_liking')} - c{searchcell(cname,'Natural-pre_liking')}; 
        cname{cnum}='Decrease_v_Natural_pre_liking';
        
        cnum = length(c)+1;
        c{cnum}    = zeros(1,length(SPM.xX.name));
        c{cnum}    = c{searchcell(cname,'Decrease-pre_liking')} - c{searchcell(cname,'Health-pre_liking')}; 
        cname{cnum}='Decrease_v_Health_pre_liking';

        cnum = length(c)+1;
        c{cnum}    = zeros(1,length(SPM.xX.name));
        c{cnum}    = c{searchcell(cname,'Health-pre_liking')} - c{searchcell(cname,'Natural-pre_liking')}; 
        cname{cnum}='Health_v_Natural_pre_liking';
