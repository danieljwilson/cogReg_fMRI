function foodreg_m8_pre_liking_analyze2(subjNameList, preproc_version)
...
modelid = pathtofile(regexp(pathtofile,'_m\d+.*_analyze')+2:(regexp(pathtofile,'_analyze')-1)); %  gets model id (1_preference)
...
% convert to TSV from Matlab
    ...
    for session=1:length(files) % this loops over sessions
		...
		%predefine modulating parameters (pre-liking ratings) for three
		%trial types
        ...
        ons_modulate{1,session}{1}.name='pre_liking';   % {1} specifies the first modulator (and only in this case)       
        ons_modulate{2,session}{1}.name='pre_liking'; 
        ons_modulate{3,session}{1}.name='pre_liking';
        ...
        pre_Natural = zeros(sum(Natural),1);
        
        trial_foods = Data.Food(Natural);
        for i = 1:sum(Natural)
            trial_food_pre = strcmp(pre_food, trial_foods(i));
            pre_Natural(i) =  pre_rating(trial_food_pre);
        end
        
        pre_DecreaseDesire = zeros(sum(DecreaseDesire),1);
        
        trial_foods = Data.Food(DecreaseDesire);
        for i = 1:sum(DecreaseDesire)
            trial_food_pre = strcmp(pre_food, trial_foods(i));
            pre_DecreaseDesire(i) = pre_rating(trial_food_pre);
        end
        
        pre_HealthFocus = zeros(sum(HealthFocus),1);
        
        trial_foods = Data.Food(HealthFocus);
        for i = 1:sum(HealthFocus)
            trial_food_pre = strcmp(pre_food, trial_foods(i));
            pre_HealthFocus(i) = pre_rating(trial_food_pre);
        end
        
        ons_modulate{1,session}{1}.vec = pre_Natural;
        ons_modulate{2,session}{1}.vec = pre_DecreaseDesire;
        ons_modulate{3,session}{1}.vec = pre_HealthFocus;
