function foodreg_m2_taste_a_health_analyze2(subjNameList, preproc_version)
...
% convert to TSV from Matlab
    ...
    for session=1:length(files) % this loops over sessions
        ...
        % First modulator
        ons_modulate{1,session}{1}.name='Taste';   
        ons_modulate{2,session}{1}.name='Taste';
        ons_modulate{3,session}{1}.name='Taste';
        
        % Second modulator
        ons_modulate{1,session}{2}.name='Health';  
        ons_modulate{2,session}{2}.name='Health';
        ons_modulate{3,session}{2}.name='Health';
        ...
        taste_Natural = zeros(sum(Natural),1);
        health_Natural = zeros(sum(Natural),1);
        
        trial_foods = Data.Food(Natural);
        for i = 1:sum(Natural)
            trial_food_taste = strcmp(taste_food, trial_foods(i));
            trial_food_health = strcmp(health_food, trial_foods(i));
            taste_Natural(i) = taste_rating(trial_food_taste);
            health_Natural(i) = health_rating(trial_food_health);
        end
        
        taste_DecreaseDesire = zeros(sum(DecreaseDesire),1);
        health_DecreaseDesire = zeros(sum(DecreaseDesire),1);
        
        trial_foods = Data.Food(DecreaseDesire);
        for i = 1:sum(DecreaseDesire)
            trial_food_taste = strcmp(taste_food, trial_foods(i));
            trial_food_health = strcmp(health_food, trial_foods(i));
            taste_DecreaseDesire(i) = taste_rating(trial_food_taste);
            health_DecreaseDesire(i) = health_rating(trial_food_health);
        end
        
        taste_HealthFocus = zeros(sum(HealthFocus),1);
        ...
        for i = 1:sum(HealthFocus)
            trial_food_taste = strcmp(taste_food, trial_foods(i));
            trial_food_health = strcmp(health_food, trial_foods(i));
            taste_HealthFocus(i) = taste_rating(trial_food_taste);
            health_HealthFocus(i) = health_rating(trial_food_health);
        end
        
        ons_modulate{1,session}{1}.vec = taste_Natural;
        ons_modulate{2,session}{1}.vec = taste_DecreaseDesire;
        ons_modulate{3,session}{1}.vec = taste_HealthFocus;
        
        ons_modulate{1,session}{2}.vec = health_Natural;
        ons_modulate{2,session}{2}.vec = health_DecreaseDesire;
        ons_modulate{3,session}{2}.vec = health_HealthFocus;
