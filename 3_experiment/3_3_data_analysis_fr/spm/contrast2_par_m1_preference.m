...
%                 };
    ...
    for subjno = 1:length(subj_ids)
        ...
        cnum = length(c)+1;
        c{cnum}    = zeros(1,length(SPM.xX.name));
        c{cnum}    = c{searchcell(cname,'Decrease-Preference')} - c{searchcell(cname,'Natural-Preference')}; 
        cname{cnum}='Decrease_v_Natural_Preference';
        
        cnum = length(c)+1;
        c{cnum}    = zeros(1,length(SPM.xX.name));
        c{cnum}    = c{searchcell(cname,'Decrease-Preference')} - c{searchcell(cname,'Health-Preference')}; 
        cname{cnum}='Decrease_v_Health_Preference';

        cnum = length(c)+1;
        c{cnum}    = zeros(1,length(SPM.xX.name));
        c{cnum}    = c{searchcell(cname,'Health-Preference')} - c{searchcell(cname,'Natural-Preference')}; 
        cname{cnum}='Health_v_Natural_Preference';
